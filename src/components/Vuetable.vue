<template>
  <table :class="[{'vuetable': true}, css.tableClass]">
    ...
    <tbody v-cloak>
    <template v-for="(groupitem, groupindex) in tableDataGroupBy">
      <tr class="table-group-header" v-if="groupBy">
        <td :colspan="fields.length" v-html="groupindex"></td>
      </tr>
      <template v-for="(item, index) in groupitem" >
        <tr @click="onRowClicked(item, $event)" :render="onRowChanged(item)"
    ...
    computed: {
      ...
      tableDataGroupBy: function () {
        ...
        for (var i = 0; i < this.tableData.length; i++) {
          ...
          var cell = this.getObjectValue(row, this.groupBy);
